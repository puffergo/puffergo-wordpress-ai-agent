...
<vault>/
  .silo/workspace.json             # SiloWorkspace(隐藏) —— 单站点布局,`silo init` 建的
  .silo/sites/<域名>.json           # 多站点布局(Obsidian 插件用;它会把上面那个迁过来并删掉原文件)
  .silo/state.json                 # 多站点布局下记住当前活动站点
  <pillar 关键词>/
    <cluster 关键词>/
...
coreKeywords:            # 恰好 1 个
  - "solar street light"
longTailKeywords:        # ≤ 5 个（核心 1 + 长尾 5，总数 ≤ 6）
...
4. **SEO 约束**:seoTitle 30–60、seoDescription 120–160;coreKeywords 恰好 1 个、longTailKeywords ≤ 5(总数 ≤ 6);核心词进 seoTitle 和正文首段。
