...
用来做**版式**：多列排版、卡片、带背景色或背景图的区段、按钮区、页面的标题大图。存成 `.html` 文件。

**大段文字不要用它**——文章、案例的正文写 Markdown（见 `prose.md`），那样客户在编辑器里才能自己改字。
...
- **只写静态 HTML**：不写 `<script>`、`<style>`、`<iframe>`、`<form>`，不写 Alpine.js（`x-data`、`@click` 等）和 `onclick` 这类属性。要放视频，别写在版式里：把正文拆开，在 `.md` 里单独一行放视频链接（见 `prose.md`）。
