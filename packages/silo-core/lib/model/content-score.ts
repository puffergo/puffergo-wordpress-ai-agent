...
import { TITLE_MIN, TITLE_MAX, DESC_MIN, DESC_MAX, seoWidth, widthRange } from './seo-limits';
...
export function computeContentScore(item: ContentItem): ContentScore {
  ...
  const titleOk = !isBlank(title) && seoWidth(title) >= TITLE_MIN && seoWidth(title) <= TITLE_MAX;
  const descOk = !isBlank(desc) && seoWidth(desc) >= DESC_MIN && seoWidth(desc) <= DESC_MAX;
  ...
  const items: ContentScoreItem[] = [
    ...
    {
      id: 'title_length',
      label: `标题长度 ${widthRange(TITLE_MIN, TITLE_MAX)}`,
      score: titleOk ? 100 : 0,
      applicable: true,
    },
    {
      id: 'desc_length',
      label: `描述长度 ${widthRange(DESC_MIN, DESC_MAX)}`,
      score: descOk ? 100 : 0,
      applicable: true,
    },
